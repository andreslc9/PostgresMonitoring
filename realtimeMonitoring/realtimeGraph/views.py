...
class AvgMeasurementDataView(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        measurement_name = request.GET.get('measurement', 'Temperatura')  # Default to 'Temperatura'
        ...
        avg_data = self.get_avg_measurement_data(city, state, country, measurement_name, start_date, end_date)
        
        return JsonResponse({'average': avg_data})

    def get_avg_measurement_data(self, city, state, country, measurement_name, start_date, end_date):
        try:
            cityO = City.objects.get(name=city)
            stateO = State.objects.get(name=state)
            countryO = Country.objects.get(name=country)
            ...
            return avg_data['value__avg'] if avg_data['value__avg'] else 0
        
        except Exception as e:
            print(f"Error obtaining average data: {e}")
            return 0
